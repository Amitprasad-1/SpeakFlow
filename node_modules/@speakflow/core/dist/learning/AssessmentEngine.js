...
const PhonemeWeaknessTracker_js_1 = require("./PhonemeWeaknessTracker.js");
class AssessmentEngine {
    /**
     * Evaluates a spontaneous speaking prompt attempt with honest metric observation.
     */
    static evaluateSpeakingAttempt(prompt, spokenTranscript, durationMs) {
     ...
     */
    static evaluateReadingAttempt(passage, spokenTranscript, durationMs) {
     ...
     */
    static evaluatePronunciationAttempt(soundGroup, targetPair, spokenTranscript, attemptNumber = 1) {
     ...
     */
    static createInitialFocusAreas(assessment) {
        const strengths = [];
        const focusAreas = [];
        ...
        if (!identifiedWeaknesses.includes('R_L'))
            identifiedWeaknesses.push('R_L');
        if (!identifiedWeaknesses.includes('TH'))
            identifiedWeaknesses.push('TH');
        if (!identifiedWeaknesses.includes('S_SH'))
            identifiedWeaknesses.push('S_SH');
     ...
     */
    static createInitialPracticePlan(assessment) {
        const focus = this.createInitialFocusAreas(assessment);
        const duration = typeof assessment.dailyPracticePreference === 'number'
     ...
     */
    static buildInitialLearnerProfile(assessment) {
        const focus = this.createInitialFocusAreas(assessment);
        const defaultSkills = PhonemeWeaknessTracker_js_1.PhonemeWeaknessTracker.createDefaultSkillProfile();
        // Calibrate initial scores based on self-reported level and assessment
        let baseScore = 65;
        if (assessment.selfReportedLevel === 'Beginner')
            baseScore = 48;
        else if (assessment.selfReportedLevel === 'Elementary')
            baseScore = 55;
        else if (assessment.selfReportedLevel === 'Intermediate' || assessment.selfReportedLevel === 'Not sure')
            baseScore = 68;
        else if (assessment.selfReportedLevel === 'Upper Intermediate')
            baseScore = 78;
        else if (assessment.selfReportedLevel === 'Advanced')
            baseScore = 86;
        ...
        const levelMapping = assessment.selfReportedLevel === 'Beginner'
            ? 'Beginner'
...
//# sourceMappingURL=AssessmentEngine.js.map