...
export class PronunciationEvaluator {
   ...
   */
  public static evaluate(input: SpeechEvaluationInput): SpeechFeedback {
  ...
  private static alignWords(
    targetWords: string[],
    spokenWords: string[],
    timestamps?: { word: string; timestamp: number }[]
  ): WordAlignment[] {
  ...
  private static findLookahead(word: string, list: string[], startIndex: number, maxLook: number): number {
   ...
   */
  public static calculateWordSimilarity(a: string, b: string): number {
    if (!a && !b) return 1.0;
    if (!a || !b) return 0.0;
    if (a === b) return 1.0;
  ...
  private static detectWeakPhonemes(
    ...
    const weak = new Set<PhonemeCategory>();
   ...
   */
  private static generateCoachingFeedback(stats: {
    ...
    if (stats.wordsPerMinute < 110) {
      tips.push(
        `Your speaking pace was ${stats.wordsPerMinute} WPM (deliberate). Aim to link prepositions and smooth out pauses to reach a natural conversational pace of 130-150 WPM.`
      );
    } else if (stats.wordsPerMinute > 175) {
      tips.push(
        `Your speaking pace was ${stats.wordsPerMinute} WPM (rapid). Slow down slightly on multisyllabic terms to ensure consonants don't blend together.`
      );
    } else {
      tips.push(
        `Great tempo! At ${stats.wordsPerMinute} WPM, your pacing is natural and easy for listeners to follow.`
      );
    }
    ...
    if (stats.hesitatedCount > 2) {
      tips.push(
        `We noticed ${stats.hesitatedCount} hesitation pauses. Try reading ahead with your eyes to anticipate the upcoming clause before voicing it.`
      );
    }
    ...
    if (stats.identifiedWeakSounds.includes('R_L')) {
      tips.push(
        'Focus on R vs L distinction: ensure your tongue tip does NOT touch the roof of the mouth for /r/, but firmly taps behind the front teeth for /l/.'
      );
    }
    if (stats.identifiedWeakSounds.includes('TH')) {
      tips.push(
        'Watch the /th/ sound: gently place the tip of your tongue between your teeth and exhale air steadily without biting.'
      );
    }
    if (stats.identifiedWeakSounds.includes('V_W')) {
      tips.push(
        'For /v/, rest your top teeth on your lower lip with vocal vibration. For /w/, round your lips into an "O" shape without teeth contact.'
      );
    }
    ...
    if (stats.skippedWords.length > 0) {
      tips.push(
        `Pay attention to connector words: you skipped "${stats.skippedWords.slice(0, 3).join('", "')}". Every syllable contributes to clear rhythm.`
      );
    }
