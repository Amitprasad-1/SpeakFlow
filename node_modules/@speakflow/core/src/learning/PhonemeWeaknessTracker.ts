import { PhonemeCategory, UserSkillProfile, SessionResult } from '../types/index.js';

export class PhonemeWeaknessTracker {
  /**
   * Updates user skill profile dynamically after a practice session.
   */
  public static updateProfileFromSession(
    profile: UserSkillProfile,
    result: SessionResult
  ): UserSkillProfile {
    const updated: UserSkillProfile = JSON.parse(JSON.stringify(profile));
    ...
    const weakList: PhonemeCategory[] = [];
    const sortedEntries = Object.entries(updated.soundSkills).sort(
      ([, a], [, b]) => a.accuracy - b.accuracy
    );

    for (const [cat, data] of sortedEntries) {
      if (data.status === 'weak' || data.accuracy < 65) {
        weakList.push(cat as PhonemeCategory);
      }
    }

    // Always have at least 1-2 focus sounds
    if (weakList.length === 0) {
      weakList.push(sortedEntries[0][0] as PhonemeCategory);
    }
   ...
   */
  public static createDefaultSkillProfile(): UserSkillProfile {
    ...
    const soundSkills: Record<PhonemeCategory, any> = {} as any;
