...
export class ContentValidator {
   ...
   */
  public static validateReadingPassage(passage: ReadingPassage): ValidationResult {
    const errors: string[] = [];
    ...
    if (wordCount < 160 || wordCount > 200) {
      errors.push(
        `Reading passage word count violation: expected 160-200 words, but received ${wordCount} words.`
      );
    }
    ...
    if (!passage.vocabularyWords || !Array.isArray(passage.vocabularyWords)) {
      ...
      errors.push(
        `Vocabulary count violation: expected exactly 6 articulation words, received ${passage.vocabularyWords.length}.`
      );
    } else {
   ...
   */
  public static validateVocabularyWord(vocab: VocabularyWord): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
   ...
   */
  public static validateSentences(sentences: PracticeSentence[]): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
   ...
   */
  public static validateDailyLesson(lesson: DailyLesson): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
